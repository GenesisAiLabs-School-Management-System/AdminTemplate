...
export const Route = createFileRoute('/_authenticated')({
  beforeLoad: () => {
    ...
    if (!tokenExists) {
      console.log("[_authenticated] beforeLoad: No token key found, redirecting to login.");
      throw redirect({
        to: '/login',
    ...
    console.log("[_authenticated] beforeLoad: Token key found, proceeding to component render.");
...
function ProtectedLayout() {
  const { isAuthenticated, isLoading, logout } = useAuth(); // Get logout from context

  console.log(`[_authenticated] Component: Rendering. isLoading: ${isLoading}, isAuthenticated: ${isAuthenticated}`);
  ...
  if (!isAuthenticated) {
     console.log("[_authenticated] Component: NOT authenticated after load (token invalid/expired?). Clearing tokens and redirecting...");
